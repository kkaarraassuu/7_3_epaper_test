...
- スマートフォンのブラウザからJPEG、PNG、WebP、HEIC/HEIFを選択・表示
...
本プロジェクトは`esptool`で書き込みます。書き込み待機にならない場合は、いったんシリアル
モニターを閉じ、Nano ESP32の`B1`と`GND`をジャンパ線で接続した状態で`RST`を押します。
その後ジャンパ線を外し、`pio run -t upload`を実行してください。
...
iOS 17以降のSafariはHEICを直接読み込めるため、写真ライブラリ内のHEIC/HEIFをそのまま選択
できます。画像の向きもSafariが保持した状態でCanvasへ描画します。

古いiOSでHEICを読み込めない場合は、iPhoneの共有メニューやショートカットでJPEGへ変換してから
選択してください。HEICデコーダーを外部CDNから取得する方式は採用していないため、設定用Wi-Fiが
インターネットへ接続されていない状態でも、対応Safariでは変換できます。
...
> [!NOTE]
> スマートフォン版もブラウザ上でCIELAB/CIEDE2000、Stucki誤差拡散、エッジ適応、軽い肌色補正を
> 実行します。端末によって変換に数秒から数十秒かかりますが、処理中の進捗を画面に表示します。
